...
import utility.prename.Prename;
public class  RteMatchDataInsolventByPolicy  implements  Task
{
	Mrecord ord;
        Mrecord whl;
	Mrecord ind;
	...
	public Result execute(Object param) 
		...
		try {
			...
			for (int i = 0 ; i < vdata.size();i++)
			{
				...
				if(data[6].trim().length() == 8)
				{					
					 searchMasterToInsolvent(data[6],data,ind,"I","A");
					 searchMasterToInsolvent(data[6],data,ord,"O","A");
					 searchMasterToInsolvent(data[6],data,whl,"W","A");
					 searchMasterULToInsolvent(data[6],data,ul,"U","A");
				}
			}
		}
		catch (Exception e)
	...
	private boolean  searchMasterToInsolvent(String policyNo,String [] data,Mrecord mrec,String policyType,String match) throws Exception 
	{
		mrec.start(0);
		for (boolean st = mrec.equal(policyNo) ; st && policyNo.compareTo(mrec.get("policyNo")) == 0 ;st=mrec.next())
		{
			System.out.println(policyNo +"   search in master "+policyType+" "+mrec.get("policyNo")+"---"+mrec.get("policyStatus1")+"---"+mrec.get("policyStatusDate1"));
			if (!name.equal(mrec.get("nameID")))
				throw new Exception(M.stou("ไม่พบข้อมูลชื่อ ของกรมธรรม์")+mrec.get("policyNo"));	
			if ("N".indexOf(mrec.get("policyStatus1")) >= 0 && mrec.get("policyStatusDate1").compareTo(DateInfo.sysDate()) != 0)
			{
				...
				("E".indexOf(mrec.get("policyStatus1")) >= 0 && mrec.get("policyStatusDate2").compareTo(beforeMatDate(mrec.get("nameID"),mrec)) == 0) ||
				...
				("LOY".indexOf(mrec.get("policyStatus1")) >= 0 && nextYear(mrec.get("policyStatusDate1"),"5").compareTo(sysDate) >= 0) )
			{
				...
				if (!insolvent.equal(mrec.get("policyNo")+data[4]))
				{
					...
					insolvent.set("idNo2","0000000000000");
					insolvent.set("preName2",data[0].trim());
					insolvent.set("firstName2",data[1].trim());
					insolvent.set("lastName2",data[2].trim());
					...
					if("AI".indexOf(match) >= 0)
					{
						...
						if(!person.equal(name.get("personID"))) 
							throw new Exception (M.stou("ไม่มีข้อมูล บุคลากร (person) "));
	...
	private boolean  searchMasterULToInsolvent(String policyNo,String [] data,Mrecord mrec,String policyType,String match) throws Exception 
	{
		mrec.start(0);
		for (boolean st = mrec.equal(policyNo) ; st && policyNo.compareTo(mrec.get("policyNo")) == 0 ;st=mrec.next())
		{
	...
	{
               // Msg.msg(new Mframe(""),"SEARCH BY POLICY to insert people");
                if (data[3].trim().length() == 0 && data[1].trim().length() == 0 && data[2].trim().length() == 0 )
                        return ;
                 
                if (data[3].trim().length() == 0)
                    data[3] = "0000000000000";       
	...
	private void insertInsolventTrans(Mrecord mrec,String [] data,String status2,String typeTran) throws Exception
	{
		...
		insolventtran.set("typeOfTran","P");
