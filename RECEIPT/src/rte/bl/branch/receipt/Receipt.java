...
public class Receipt
{
	...
	static String systime ="";
        ...
        public static final String sai5 = "N215Y";
        public static final String sai6 = "N216U";
        public static final String sai3 = "N213U";        
	public Receipt(String type,String yy) throws Exception
	{
		typeOfRp = type;
		...
		Mrecord rprider = null;
                ...
                if(send.equal(typeOfRp+sbranch))
                {
                 ...
                 /*----------  	 if(Masic.echo("bra"+send.get("branch")+"app","hi") != 0)
                        ...
                        System.out.println("Sending data to "+send.get("branch")+"  "+sai+"  "+send.get("wantToPrint")+ "  "+send_filename);
		...
		/*		if("255201".compareTo(served.get("effectiveDate").substring(0,6)) > 0)
				...
				if(served.get("rpFlag").charAt(0) != '0' )
				{
					...
					continue;
                                ...
                                {
                                      //  System.out.println(" before check in "+sai+"   "+served.get("policyNo")+"  "+served.get("agent"));
                ...
                for (boolean st = crprid.equalGreat(key) ; st && key.compareTo(crprid.get("rpNo")) == 0 ; st=crprid.next())                {
                        ...
                        while (j < len)
                        {
                                if(riderText.substring(j,j+3).compareTo(M.stou("ฉพ ")) == 0 ||
                                        ...
                                        riderText.substring(j,j+3).compareTo(M.stou("ฉพ3")) == 0 ||
                                        riderText.substring(j,j+3).startsWith(M.stou("ฉพ")))
                                {
	...
	private String getRange(String branch) throws Exception
	{
		...
		String trp = "0000001";
                String msg = "";
                if (branch.compareTo("A00") == 0)
                        tyear = M.setlen(M.subnum(tyear,"57"),2);

		rprange.lock();
		if(rprange.equal(branch+tyear)) 
		{
		...
		return (branch+tyear+trp);
	}
        public String checkForAPLRctrl (String policyNo,String payPeriod,String mode) throws Exception
        {
               if (Masic.fileStatus("aplrctrl@mstscan") < 2)
                    return "000000000000";    
              //Msg.msg(new Mframe(""),policyNo+"  "+payPeriod+"  "+mode);
                System.out.println("In checkForAPLRctrl ---------- "+policyNo+"  "+payPeriod+"  "+mode);
                Mrecord aplrc = CFile.opens("aplrctrl@mstscan");
                aplrc.start(1);
                if (aplrc.equalGreat(policyNo+payPeriod) && (policyNo+payPeriod).compareTo(aplrc.get("policyNo")+aplrc.get("payPeriod")) == 0)
                {
                        System.out.println("In If checkForAPLRctrl ---------- "+policyNo+"  "+payPeriod+"  "+mode+" --"+aplrc.get("mode")+"  "+aplrc.get("deductTax"));
                        if (mode.compareTo(aplrc.get("mode")) == 0)
                        {
                                if (aplrc.get("deductTax").charAt(0) == 'Y')
                                        return aplrc.get("rpNo");
                                else
                                        return "000000000000"; 
                        }
                        if (payPeriod.substring(2,4).compareTo("01") == 0)
                        {
                                if (aplrc.get("deductTax").charAt(0) == 'Y')
                                        return aplrc.get("rpNo");
                                else
                ...
                if (mode.charAt(0) != '4')
                {
                       ...
                       payPeriod = payPeriod.substring(0,2)+"03";
                System.out.println("In checkForAPLRctrl -----row2----- "+policyNo+"  "+payPeriod+"  "+mode);
                       if (aplrc.equalGreat(policyNo+payPeriod) && (policyNo+payPeriod).compareTo(aplrc.get("policyNo")+aplrc.get("payPeriod")) == 0)
                       { 
                System.out.println("In if checkForAPLRctrl -----row2----- "+policyNo+"  "+payPeriod+"  "+mode);
                                if (aplrc.get("mode").charAt(0) == '4')
                                {
                                        if (aplrc.get("deductTax").charAt(0) == 'Y')
                                                 return aplrc.get("rpNo");
                                        else
        ...
        private String changePrintModeToMasterMode(String modePrint)
        {
                String masterMode = "0";
                switch (M.ctoi(modePrint))
                {
                        case  1 : masterMode = "0";
                                 break;
                        case  6 : masterMode = "2";
...
//		if("XA".indexOf(flagPrint) >= 0 && typeOfRp.charAt(0) != 'I')
		...
		System.out.println("rpprint.........................................<><><> size == "+rpprint.size());
                Record tempserv = served.copy();
		for (int i = 0 ; i < rpprint.size();i++)
		{
                          tempserv.putBytes((byte [])rpprint.elementAt(i));

                          String  rpNoFromAPL = "000000000000";
                          if("XAFG".indexOf(flagPrint) < 0)
                                rpNoFromAPL = checkForAPLRctrl(tempserv.get("policyNo"),tempserv.get("payPeriod"),changePrintModeToMasterMode(tempserv.get("mode")));
                          if (rpNoFromAPL.compareTo("000000000000") != 0 &&  rc.equal(rpNoFromAPL))
                                 throw new Exception(M.stou("ไม่สามารถออกใบเสร็จเบี้ยประกันภัยได้ ติดต่อส่วนสนับสนุนระบบงานบริการฯ เบอร์ 2642 , 2643 "));                   
                } 
		for (int i = 0 ; i < rpprint.size();i++)
		{
			...
			String oldrpno = served.get("rpNo");
                        if("XAFG".indexOf(flagPrint) < 0)
                                rpNoFromAPL = checkForAPLRctrl(served.get("policyNo"),served.get("payPeriod"),changePrintModeToMasterMode(served.get("mode")));
                        if (rpNoFromAPL.compareTo("000000000000") == 0)
			        served.set("rpNo",getRange(branch));
                        else {
                                if (rc.equal(rpNoFromAPL))
                                   
                                        rpNoFromAPL = getRange("A00");
			        served.set("rpNo",rpNoFromAPL);
                        }
			...
			else if("XAFG".indexOf(flagPrint) >= 0 && typeOfRp.charAt(0) != 'I')
			{
	...
	private void  moveToInsertCtrl(Mrecord rc ,Record served,String month,Mrecord rprider,Mrecord crprider,String flagPrint,String branch,String userID) throws Exception
	{
		...
		if(flagPrint.charAt(0) == 'A' || flagPrint.charAt(0) == 'F' || flagPrint.charAt( 0 ) == 'G' && M.cmps( branch, "000") != 0)
		{
			rc.set("originalStatus","A");
			rc.set("currentStatus","A");
			if ( flagPrint.charAt(0) == 'G')
				rc.set("reasonCode", "CG");
			else if ( flagPrint.charAt(0) == 'F')
		...
		if (to_stat.charAt(0) == 'G')		
			rc.set("reasonCode","CG");
		else if (to_stat.charAt(0) == 'O') // rider offer 
		{
			rc.set("reasonCode","RR");
		}	
		if(!rc.insert())
...
//		sysdate = "25510412";
	...
	public static boolean isOneYearTerm(String planCode)
        {
                String [] otPlan = {"TE01","TE02","TE07","TW1"};
                for (int i = 0 ; i < otPlan.length ; i++)
                {
                        if (planCode.compareTo(otPlan[i]) == 0)
                                return true;
                }
                return false ;
        }
                ...
                switch (receiptFlag.charAt(0))
                {
					...
					if(ord.get("policyStatus2").charAt(0) == 'B' || ord.get("oldPolicyStatus2").charAt(0) == 'B')
					{
						...
						dueDate = Insure.dueDate(ord.get("mode"),ord.get("effectiveDate"),pPeriod);
                                        ...
                                        if(insure.PlanType.isPAPlan(ord.get("planCode")) || ord.get("policyStatus1").charAt(0) == 'F' ||  isOneYearTerm(ord.get("planCode")) )
                                                day = 31;
                                        master = ord ;
                                   }
                                  break;
...
//                      System.out.println("25274344...........................diff ---"+diff);
	...
	public static void main(String [] args) throws Exception
	{
		...
		Receipt rpserved = new Receipt(typeOfPol,month.substring(2,4));
		if(args.length == 2)
