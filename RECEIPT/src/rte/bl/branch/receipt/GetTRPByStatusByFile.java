...
public class  GetTRPByStatusByFile  implements  InterfaceRpt
{
        Mrecord trec;
	public void makeReport(String [] args) throws Exception
		...
		String startDate = "";
		if (period.substring(6).compareTo("15") == 0)
			startDate = period.substring(0,6)+"01";
		else	
			startDate = period.substring(0,6)+"16";
		...
		for (int i = 0 ; i < status.length();i++)
		{
			...
			String ss = startDate;
			while (ss.compareTo(period) <= 0)
			{
				getData(tstatus,ss,branch);
				ss = M.nextdate(ss,1);
			}
		}
