...
public class RteClearReceiptNew implements Task
{
	Mrecord trptmp;
			...
			while (st == true && (ask.get("branch")+ask.get("rpNo")).compareTo(branch+trpno.substring(2,14)) == 0)
			{
				...
				switch (status){
					...
					case 'A' : if (CheckAskReceipt.checkUCStatus(urc,ask.get("rpNo"),ask.get("requestDate")))
						 {
							System.out.println("found in urc");
							if (trpno.charAt(15) != ' ') 
							{
								if(reasonCode.compareTo("00") != 0)
									urc.set("reasonCode",reasonCode);
								insertXrcchg(xrcchg,'U',urc.copy(),trpno.substring(15,16));
								urc.set("currentStatus",trpno.substring(15,16));
                      						urc.set("sysDate",sysdate);
								urc.set("userID",userID);
						//		if(reasonCode.compareTo("00") != 0)
						//			urc.set("reasonCode",reasonCode);
							}
							else {
								urc.set("requestDate","00000000");
								if("UD".indexOf(urc.get("currentStatus")) >= 0)
									urc.set("currentStatus",urc.get("originalStatus"));
							}
            						urc.update();
							deleteRpAskFor(ask.get("receiptFlag"),ask.get("rpNo"),ask.get("requestDate"));
        						ask.set("receiptFlag","B");
							ask.update();
							if(trpno.charAt(15) == 'G')
							{
								insertDataToPolicyNotReq(trpno);
