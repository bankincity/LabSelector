...
public class AUCRPYYYYMM 
{
	...
	String currentMonth ;
	public AUCRPYYYYMM() throws Exception
	{
...
//		ucrp = CFile.opens("ucrp"+month+"@cbranch");
		...
		{
			System.out.println("ucrp................."+ ucrp.get("rpNo"));
			if(++count % 1000 == 0)
...
//					trec = getReceiptRecord(ucyymm.get("rpNo"),ucyymm.get("receiptFlag"));
			...
			for (boolean st = ucyymm.first(); st; st = ucyymm.next())
			{
					...
					ss = ucdetail.next())
				{
					...
					if (ucdetail.get("currentStatus").charAt(0) != 'S')
						updateCurrentStatus();
					 System.out.println(" -- >> ucdetail.currentStatus:"+ ucdetail.get("currentStatus")); 
					 if(cmpMonth.compareTo(month) == 0 ||  "DU".indexOf(ucdetail.get("currentStatus")) >= 0 )
					{
						//&& ucdetail.get("payDate").compareTo("00000000") == 0)
	...
	{		
		String [] field = {"policyNo", "rpNo", "payPeriod", "premium", "currentStatus", "payDate", "submitNo", 
						   "isClear", "receiptFlag"};
	...
	private Record getReceiptRecord (String rpNo,String receiptFlag,String policyNo)
	{
		...
		else if(receiptFlag.charAt(0) == 'U')
		{
			if(urc.equal(rpNo))
			{
				trec = urc;
